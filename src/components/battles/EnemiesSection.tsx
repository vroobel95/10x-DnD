  ...
  async function handleGenerate(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setPending((prev) => [...(data.enemies as Enemy[]), ...prev]);
        setPrompt("");
      }
    } catch {
      ...
      <section>
        ...
        <form onSubmit={handleGenerate} className="space-y-3">
          <textarea
            ...
            className="w-full resize-none rounded-lg border border-white/10 bg-white/5 px-4 py-3 text-sm text-white placeholder-blue-100/30 focus:border-purple-500/50 focus:outline-none focus:ring-1 focus:ring-purple-500/50 disabled:opacity-50"
          />
          {generateError && (
