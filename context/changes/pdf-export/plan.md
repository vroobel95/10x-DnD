...
- [x] 1.8 Generated bytes open as a valid PDF with expected per-enemy pages and readable layout
...
- [x] 2.3 Integration test: 500 when `createClient` returns null
- [x] 2.4 Integration test: 401 when unauthenticated
- [x] 2.5 Integration test: 404 on battle not found and 500 on non-not-found battle SELECT error
- [x] 2.6 Integration test: 404 when battle not owned (campaign IDOR)
- [x] 2.7 Integration test: 404 when no confirmed enemies
- [x] 2.8 Integration test: 200 with `application/pdf` + `Content-Disposition` + `%PDF-` body

#### Manual

- [x] 2.9 Logged-in request for an owned battle downloads a valid PDF
- [x] 2.10 Requesting another user's battle id returns 404 (no data leak)
